...
# Create a DeepseekMoE from a set of equally-sized Deepseek (or Llama) models.
...
class DeepseekMOEConfig(BaseModel):
    ...
    num_experts_per_tok: int = 2  # Renamed from experts_per_token
...
def get_hidden_states(
   model: Union[DeepseekForCausalLM, LlamaForCausalLM],
...
def build(
   ...
   if not isinstance(base_cfg, DeepseekConfig):
       base_cfg_deepseek = DeepseekConfig(**base_cfg.to_dict())
       base_cfg_deepseek.sliding_window = None
       base_cfg_deepseek.max_position_embeddings = base_cfg.max_position_embeddings
       base_cfg = base_cfg_deepseek
   ...
   out_cfg.first_k_dense_replace = config.first_k_dense_replace
   out_cfg.norm_topk_prob = config.norm_topk_prob
   out_cfg.scoring_func = config.scoring_func
   out_cfg.auto_map = {
   ...
   if (out_cfg.n_routed_experts & (out_cfg.n_routed_experts - 1)) != 0:
       logging.warning(
           f"Your model has {out_cfg.n_routed_experts} experts, which is "
